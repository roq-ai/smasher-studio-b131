...
import { getStudioById, updateStudioById } from 'apiSdk/studios';
...
function StudioEditPage() {
  ...
  const handleSubmit = async (values: StudioInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await updateStudioById(id, values);
      mutate(updated);
      resetForm();
      router.push('/studios');
    } catch (error) {
  ...
  const formik = useFormik<StudioInterface>({
    initialValues: data,
