...
import { FiMail, FiUsers, FiMenu, FiUser, FiMessageCircle, FiFile, FiBox, FiFilm, FiFolder } from 'react-icons/fi';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const { data, error, isLoading } = useSWR<StudioInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/studios` : null),
    () => getStudios({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Studio',
      path: isTenantUser ? `/studios/view/${data?.[0]?.id}` : '/studios',
      entity: 'studio',
      service: AccessServiceEnum.PROJECT,
      icon: FiFilm,
    },
    { name: 'File', path: '/files', entity: 'file', service: AccessServiceEnum.PROJECT, icon: FiFile },
